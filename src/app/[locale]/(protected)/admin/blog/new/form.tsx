...
export default function CreateArticleForm({ className }: Props) {
  const locale = useLocale()
  const t = useTranslations("Components.FormArticle")
  const router = useRouter()
  const form = useForm<zArticleCreate>({
  ...
  const { execute, isExecuting } = useAction(createArticle, {
    onSuccess: ({ data }) => {
      if (data?.error) {
  ...
  function onSubmit(data: zArticleCreate) {
    execute(data)
  }
            ...
            <Tabs defaultValue={locale}>
              ...
              {AppConfig.locales.map(locale => (
                ...
                >
                  ...
                  <FormField
                    control={form.control}
                    name={`title_${locale.id}` as keyof zArticleCreate}
                    render={({ field }) => (
                  ...
                  <FormField
                    control={form.control}
                    name={`content_${locale.id}` as keyof zArticleCreate}
                    render={({ field }) => (
                      <FormItem className="xl:col-span-2">
                        <FormLabel>
                  ...
                  <FormField
                    control={form.control}
                    name={`linkTitle_${locale.id}` as keyof zArticleCreate}
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
