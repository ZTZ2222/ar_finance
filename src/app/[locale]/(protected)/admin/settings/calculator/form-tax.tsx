...
import type { zTaxSystem } from "@/types/calculator.schema"

type Props = {
  optionsData: zTaxSystem[] | null
  className?: string
}
...
export default function TaxSystemForm({ optionsData, className }: Props) {
  ...
  const { execute, isExecuting } = useAction(createTaxSystem, {
    onSuccess: ({ data }) => {
      if (data?.error) {
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 xl:col-span-2">
          {optionsData?.map((option, index) => (
            ...
            >
              <div>{option[`name_${locale}` as keyof zTaxSystem]}</div>
