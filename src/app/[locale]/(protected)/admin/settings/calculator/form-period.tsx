...
import type { zTimePeriod } from "@/types/calculator.schema"

type Props = {
  optionsData: zTimePeriod[] | null
  className?: string
}
...
export default function TimePeriodForm({ optionsData, className }: Props) {
  ...
  const { execute, isExecuting } = useAction(createTimePeriod, {
    onSuccess: ({ data }) => {
      if (data?.error) {
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 xl:col-span-2">
          {optionsData?.map((option, index) => (
            ...
            >
              <div>{option[`period_${locale}` as keyof zTimePeriod]}</div>
        ...
        {optionFields.map((optionField, index) => (
          ...
          >
            ...
            <FormField
              control={form.control}
              name={`options.${index}.period_ru` as const}
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input
            ...
            <FormField
              control={form.control}
              name={`options.${index}.period_en` as const}
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input
            ...
            <FormField
              control={form.control}
              name={`options.${index}.period_ky` as const}
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input
