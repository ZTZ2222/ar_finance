...
import type { zFieldOfActivity } from "@/types/calculator.schema"

type Props = {
  optionsData: zFieldOfActivity[] | null
  className?: string
}

type FormValues = {
  options: zFieldOfActivity[]
}

export default function FieldOfActivityForm({ optionsData, className }: Props) {
  ...
  const newOption: zFieldOfActivity = {
    name_ru: "",
    name_en: "",
    name_ky: "",
  }
  ...
  const { execute, isExecuting } = useAction(createFieldOfActivity, {
    onSuccess: ({ data }) => {
      if (data?.error) {
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 xl:col-span-2">
          {optionsData?.map((option, index) => (
            ...
            >
              <div>{option[`name_${locale}` as keyof zFieldOfActivity]}</div>
